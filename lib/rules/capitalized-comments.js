...
function getNormalizedOptions(rawOptions, which) {
    ...
    return rawOptions[which] || rawOptions;
}
...
function createRegExpForIgnorePatterns(normalizedOptions) {
    Object.keys(normalizedOptions).forEach(function(key) {
        ...
        if (ignorePatternStr) {
            const regExp = RegExp(`^(?:${ignorePatternStr})`);
...
module.exports = {
    ...
    create(context) {
        ...
        function processComment(comment) {
            ...
            if (options.ignorePatternRegExp && options.ignorePatternRexExp.test(commentWithoutAsterisks)) {
                return;
            }
